...
const topologiaRed = {
  ...
  // Recorremos cada conexión y aumentamos el contador de ambos nodos (origen y destino)
  ...
  const nodosOrdenados = Object.entries(conexionesPorNodo)
    .sort((a, b) => b[1] - a[1]); // Orden descendente por número de conexiones
  
  // Sugerencias de optimización
  const sugerencias = [];
  for (const [nodo, totalConexiones] of Object.entries(conexionesPorNodo)) {
    if (totalConexiones > 2) {
      sugerencias.push(`Nodo ${nodo} tiene ${totalConexiones} conexiones: considerar aumentar ancho de banda o balanceo de tráfico`);
    }
  }
